...
export default function Header() {
  ...
  return (
    <header className="w-full bg-black/90 text-white font-montserrat relative z-50">
      ...
      {/* TIMESTAMP — NOW FULLY PIMPED + VISIBLE SUN */}
      <div className="relative w-full py-3 overflow-hidden border-b border-white/10">

        {/* Distant sun — now bright and noticeable */}
        <div className="absolute top-1/2 left-1/2 w-[260px] h-[260px] 
          -translate-x-1/2 -translate-y-1/2 
          bg-yellow-300/20 blur-2xl rounded-full 
          animate-bigSun pointer-events-none">
        </div>

        {/* Decorative shine layer */}
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shine"></div>

        {/* Timestamp text */}
        <div className="relative text-center text-sm tracking-wider font-semibold 
          bg-gradient-to-r from-green-400/20 via-green-400/10 to-green-400/20 
          py-[6px] border-y border-green-400/20 
          backdrop-blur-md rounded-none shadow-lg 
          animate-timeLift">
          {timestamp}
        </div>
      </div>
      ...
      <nav className="hidden md:flex justify-between items-center px-10 py-3 border-t border-white/10">
        <ul className="flex gap-10 text-sm font-montserrat tracking-wide">
          ...
          <li><a onClick={() => goTo("/programs")} className="cursor-pointer hover:text-green-400 transition">Programs</a></li>
          <li><a onClick={() => goTo("/friends")} className="cursor-pointer hover:text-green-400 transition">Friends</a></li>
          <li><a onClick={() => goTo("/community")} className="cursor-pointer hover:text-green-400 transition">Community</a></li>
      ...
      {mobileMenuOpen && (
        <div className="absolute w-[90%] left-1/2 -translate-x-1/2 mt-2 rounded-3xl backdrop-blur-xl bg-black/30 p-6 shadow-2xl border border-white/20 font-montserrat">
          <ul className="flex flex-col items-center gap-4 text-base">
            ...
            <li><a onClick={() => goTo("/programs")} className="hover:text-green-400 transition">Programs</a></li>
            <li><a onClick={() => goTo("/friends")} className="hover:text-green-400 transition">Friends</a></li>
            <li><a onClick={() => goTo("/community")} className="hover:text-green-400 transition">Community</a></li>
      ...
      <style>
        {`
          ...
          .animate-bigSun {
            animation: bigSun 7s ease-in-out infinite;
          }

          .animate-shine {
            animation: shine 2s ease-in forwards;
          }

          .animate-timeLift {
            animation: timeLift 4s ease-in-out infinite;
          }
        `}
      </style>
    </header>
